 ...
 ***************************************************************/
/**
 * Class used in module web (advanced search) and which may hold code specific for that module
 * However the class has a general principle in it which may be used in the web/export module.
 *
 * $Id: class.tx_joh_advbesearch.php,v 0.10 2005/11/4 typo3 Exp $
 * @author of the original fullsearch Kasper Skaarhoj <kasperYYYY@typo3.com>
 * @author of this modified version JoH asenau <info@cybercraft.de>
 */
/**
 ...
 *
 *
 *   87: class tx_johadvbesearch
 *   98:     function form()
 *  112:     function makeStoreControl()
 *  151:     function initStoreArray()
 *  169:     function cleanStoreQueryConfigs($storeQueryConfigs, $storeArray)
 *  186:     function addToStoreQueryConfigs($storeQueryConfigs, $index)
 *  202:     function saveQueryInAction($uid)
 *  247:     function loadStoreQueryConfigs($storeQueryConfigs, $storeIndex, $writeArray)
 *  263:     function procesStoreControl()
 *  335:     function queryMaker()
 *  400:     function getQueryResultCode($mQ, $res, $table)
 ...
 *  656:     function getProcessedValueExtra($table, $fN, $fV, $conf, $splitString)
 *  774:     function getTreeList($id, $depth, $begin = 0, $perms_clause)
 *  811:     function makeValueList($fN, $fV, $conf, $table, $splitString)
 * 1022:     function resultRowTitles($row, $conf, $table)
 * 1054:     function csvRowTitles($row, $conf, $table)
 *
 * TOTAL FUNCTIONS: 19
 ...
 * @subpackage t3lib
 */
class tx_johadvbesearch
{
    var $storeList = 'search_query_smallparts,search_result_labels,labels_noprefix,queryConfig,queryTable,queryFields,queryLimit,queryOrder,queryOrderDesc,queryOrder2,queryOrder2Desc,queryGroup,search_query_makeQuery';
    ...
    function makeStoreControl()
    {
        ...
        foreach ($storeArray as $k => $v) {
            $opt[] = '<option value="' . $k . '"' . (!strcmp($cur,
                    $v) ? " selected" : "") . '>' . htmlspecialchars($v) . '</option>';
        }
        ...
        if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('sys_action')) {
            ...
            if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
                $opt[] = '<option value="0">__' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:savetoaction') . ':__</option>';
                while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
                    $opt[] = '<option value="-' . $row['uid'] . '"' . (!strcmp($cur,
                            "-" . $row['uid']) ? " selected" : "") . '>' . htmlspecialchars($row['title'] . ' [' . $row['uid'] . ']') . '</option>';
                }
            }
        ...
        $tmpCode = '
				...
				<tr' . $TDparams . '><td><select name="storeControl[STORE]" onChange="document.forms[0][\'storeControl[title]\'].value= this.options[this.selectedIndex].value!=0 ? this.options[this.selectedIndex].text : \'\';">' . implode(chr(10),
                $opt) . '</select><input type="submit" name="storeControl[LOAD]" value="' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:load') . '"></td></tr>
				<tr' . $TDparams . '><td nowrap><input name="storeControl[title]" value="" type="text" max=80' . $GLOBALS['SOBE']->doc->formWidth() . '><input type="submit" name="storeControl[SAVE]" value="' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:save') . '" onClick="if (document.forms[0][\'storeControl[STORE]\'].options[document.forms[0][\'storeControl[STORE]\'].selectedIndex].value<0) return confirm(\'' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:savewarn') . '\');"><input type="submit" name="storeControl[REMOVE]" value="' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:remove') . '"></td></tr>
				</table>
				';
    ...
    function initStoreArray()
    {
        ...
        $storeArray = array(
            '0' => '[' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:new') . ']'
        );
    ...
    function saveQueryInAction($uid)
    {
        if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('sys_action')) {
            ...
            if ($saveArr['queryTable']) {
                $qGen = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_johadvbesearch_queryGenerator');
                $qGen->init('queryConfig', $saveArr['queryTable']);
                $qGen->makeSelectorTable($saveArr);

                $qGen->enablePrefix = 1;
                $qString = $qGen->getQuery($qGen->queryConfig);
                $qCount = $GLOBALS['TYPO3_DB']->SELECTquery('count(*)', $qGen->table,
                    $qString . t3lib_BEfunc::deleteClause($qGen->table));
                ...
                if (!$GLOBALS['TYPO3_DB']->sql_error()) {
                    ...
                    if (!empty($GLOBALS['_POST']['storeControl']['title'])) {
                        $dA['title'] = $GLOBALS['_POST']['storeControl']['title'];
                    }
    ...
    function procesStoreControl()
    {
        ...
        if ($saveStoreArray) {
            ...
            $writeArray['storeQueryConfigs'] = serialize($this->cleanStoreQueryConfigs($storeQueryConfigs,
                $storeArray));
            $GLOBALS['SOBE']->MOD_SETTINGS = t3lib_BEfunc::getModuleData($GLOBALS['SOBE']->MOD_MENU, $writeArray,
                $GLOBALS['SOBE']->MCONF['name'], 'ses');
        }
    ...
    {
        global $TCA;
        if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['joh_advbesearch'])) {
            $this->hookArray = $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['joh_advbesearch'];
        }
        $msg = $this->procesStoreControl();
	
        if (!$GLOBALS['BE_USER']->userTS['advBESearch.']['disableStoreControl']) {
            $output .= $GLOBALS['SOBE']->doc->section($GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:loadsavequery'),
                $this->makeStoreControl(), 0, 1);
            if ($msg) {
        ...
        if (!$GLOBALS['BE_USER']->userTS['advBESearch.']['disableMakeQuery']) {
            $output .= $GLOBALS['SOBE']->doc->section($GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:makequery'),
                $tmpCode, 0, 1);
        }
        ...
        if ($qGen->table && is_array($TCA[$qGen->table])) {
            ...
            if ($mQ) {
                ...
                switch ($mQ) {
                    case 'count':
                        $qExplain = $GLOBALS['TYPO3_DB']->SELECTquery('count(*)', $qGen->table,
                            $qString . t3lib_BEfunc::deleteClause($qGen->table));
                        break;
                    default:
                ...
                if (!$GLOBALS['BE_USER']->userTS['advBESearch.']['disableShowSQLQuery']) {
		    
                    $output .= $GLOBALS['SOBE']->doc->section($GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:sqlquery'),
                       
		      $this->tableWrap(htmlspecialchars($qExplain)), 0, 1);
		...
		if ($GLOBALS['TYPO3_DB']->sql_error()) {
		    $out .= '<BR><strong>Error:</strong><BR><font color=red><strong>' . $GLOBALS['TYPO3_DB']->sql_error() . '</strong></font>';
                    $output .= $GLOBALS['SOBE']->doc->section($GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:sqlerror'),
                        $out, 0, 1);
                } else {
    ...
    {
        global $TCA;
        if (!empty($TCA[$table]['ctrl']['EXT']['realTable'])) {
            $altTable = $TCA[$table]['ctrl']['EXT']['realTable'];
        } else {
            $altTable = $table;
        }
        $output = '';
        ...
        switch ($mQ) {
            case 'count':
                ...
                $cPR['header'] = $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:count');
                //    $cPR['content'] = '<BR><strong>'.$row[0]. '</strong> records selected.';
                $cPR['content'] = '<BR><strong>' . $row[0] . '</strong> ' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:recordsselected');
                break;
            case 'all':
                $rowArr = array();
                while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
                    $rowArr[] = $this->resultRowDisplay($row, $TCA[$table], $altTable);
                    $lrow = $row;
                }
                ...
                if (count($rowArr)) {
                    $out .= "<table border=0 cellpadding=2 cellspacing=1 width=\"100%\">" . $this->resultRowTitles($lrow,
                            $TCA[$table], $table) . implode(chr(10), $rowArr) . '</table>';
                }
                if (!$out) {
                    $out = '<em>' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:norowsselected') . '</em>';
                }
                $cPR['header'] = $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:result');
                $cPR['content'] = $out;
                break;
            case 'csv':
                $rowArr = array();
                ...
                while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
                    if ($first) {
                        $rowArr[] = $this->csvRowTitles($row, $TCA[$table], $table);
                        $first = 0;
                    }
                    $rowArr[] = $this->csvValues($row, ',', '"', $TCA[$table], $table);
                }
                if (count($rowArr)) {
                    $out .= '<textarea name="whatever" rows="20" wrap="off"' . $GLOBALS['SOBE']->doc->formWidthText($this->formW,
                            '', 'off') . '>';
                    $out .= \TYPO3\CMS\Core\Utility\GeneralUtility::formatForTextarea(implode(chr(10), $rowArr)) . '</textarea>';
                    if (!$this->noDownloadB) {
                        $out .= '<BR><input type="submit" name="download_file" value="Click to download file" onClick="document.location=\'' . $this->downloadScript . '\';">';
                    ...
                    if (\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('download_file')) {
                        ...
                        //echo "<script type='text/javascript'>alert(".$_SERVER['HTTP_USER_AGENT'].");</script>";exit();
		...
		$xmlObj = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_johadvbesearch_t3libxml');
                $xmlObj->setTopLevelName('typo3_export');
		$xmlObj->includeNonEmptyValues = 1;
                ...
                if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
                    ...
                    $out .= '<textarea name="whatever" rows="20" wrap="off"' . $GLOBALS['SOBE']->doc->formWidthText($this->formW,
                            '', 'off') . '>' . \TYPO3\CMS\Core\Utility\GeneralUtility::formatForTextarea($xmlData) . '</textarea>';
                    if (!$this->noDownloadB) {
                        $out .= '<BR><input type="submit" name="download_file" value="Click to download file" onClick="document.location=\'' . $this->downloadScript . '\';">';
                        // document.forms[0].target=\'_blank\';
                    }
		    ...
		    $out .= '<BR>' .\TYPO3\CMS\Core\Utility\DebugUtility::viewArray($row);
                ...
                $cPR['header'] = $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:explainsqlquery');
                $cPR['content'] = $out;
                break;
        }
        ...
        if ($swords) {
            ...
            while (list($table) = each($TCA)) {
                ...
                $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('count(*)', $table,
                    $qp . t3lib_BEfunc::deleteClause($table));
                ...
                if ($count || $showAlways) {
                    ...
                    if ($count) {
                        $rowArr = array();
                        $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,' . $conf['ctrl']['label'], $table,
                            $qp . t3lib_BEfunc::deleteClause($table), '', '', $limit);
                        while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
                            $rowArr[] = $this->resultRowDisplay($row, $conf, $altTable);
                            $lrow = $row;
                        }
                        $out .= '<table border=0 cellpadding=2 cellspacing=1>' . $this->resultRowTitles($lrow, $conf,
                                $table) . implode(chr(10), $rowArr) . '</table>';
                    }
                    $out .= '<HR>';
                }
            }
    ...
    function resultRowDisplay($row, $conf, $table)
    {
        ...
        foreach ($row as $fN => $fV) {
            if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($SET['queryFields'],
                    $fN) || (!$SET['queryFields'] && $fN != 'pid' && $fN != 'deleted')
            ) {
                //   $TDparams = ' nowrap';
        ...
        if (!$row['deleted']) {
            ...
            //if (is_array(t3lib_div::_POST('SET')))
            $out .= '<a href="#" onClick="' . t3lib_BEfunc::editOnClick($params, $GLOBALS['BACK_PATH'],
                    \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REQUEST_URI')) . '"><img src="' . $GLOBALS['BACK_PATH'] . 'gfx/edit2.gif" width="11" height="12" border="0" alt=""></a>';
            if ($table == 'fe_users') {
                $out .= '<a href="http://' . $_SERVER['HTTP_HOST'] . '?logintype=login&feid=' . $row['uid'] . '" target="_blank"><img src="' . $GLOBALS['BACK_PATH'] . '../fileadmin/template/images/fe_login.gif" width="23" height="16" border="0" alt="FE-Login"></a>';
            }
        } else {
            ...
            $out .= '<img src="' . $GLOBALS['BACK_PATH'] . \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('joh_advbesearch') . 'gfx/undelete.gif" width="13" height="12" border="0" alt="' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:undelete') . '"></A>';
            $out .= '<a href="' . \TYPO3\CMS\Core\Utility\GeneralUtility::linkThisUrl($GLOBALS['BACK_PATH'] . 'tce_db.php', array(
                    'cmd[' . $table . '][' . $row['uid'] . '][undelete]' => '1',
            ...
            $out .= '<img src="' . $GLOBALS['BACK_PATH'] . \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('joh_advbesearch') . 'gfx/undelete_and_edit.gif" width="13" height="12" border="0" alt="' . $GLOBALS['LANG']->sL('LLL:EXT:joh_advbesearch/mod1/locallang_mod.php:undeleteand[M Cedit') . '">';
        }
        $_params[$table] = $row;
        ...
        if ($fieldSetup['type'] == 'relation') {
            ...
            if (stristr($fieldSetup['allowed'], ',')) {
                ...
                if (!$fieldSetup['prepend_tname']) {
                    $checkres = $GLOBALS['TYPO3_DB']->exec_SELECTquery($fN, $table,
                        'uid ' . t3lib_BEfunc::deleteClause($table), $groupBy = '', $orderBy = '', $limit = '');
                    if ($checkres) {
            ...
            foreach ($from_table_Arr as $from_table) {
                ...
                if (is_array($TCA[$from_table])) {
                    ...
                    foreach ($this->tableArray[$from_table] as $key => $val) {
                        ...
                        $prefixString = $GLOBALS['SOBE']->MOD_SETTINGS['labels_noprefix'] == 'on' ? '' :
                            ' [' . $tablePrefix . $val['uid'] . '] ';
    ...
    function resultRowTitles($row, $conf, $table)
    {
        ...
        foreach ($row as $fN => $fV) {
            if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($SET['queryFields'],
                ...
                if ($GLOBALS['SOBE']->MOD_SETTINGS['search_result_labels']) {
                    $out .= '<td' . $TDparams . '><strong>' . $GLOBALS['LANG']->sL($conf['columns'][$fN]['label'] ? $conf['columns'][$fN]['label'] : $fN,
                            1) . '</strong></td>';
                } else {
    ...
    function csvRowTitles($row, $conf, $table)
    {
        ...
        foreach ($row as $fN => $fV) {
            if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($SET['queryFields'], $fN) || (!$SET['queryFields'] && $fN != 'pid')) {
                if (!$out) {
                    if ($GLOBALS['SOBE']->MOD_SETTINGS['search_result_labels']) {
                        $out = $GLOBALS['LANG']->sL($conf['columns'][$fN]['label'] ? $conf['columns'][$fN]['label'] : $fN,
                            1);
                    } else {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/joh_advbesearch/mod1/tx_johadvbesearch.php']) {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/joh_advbesearch/mod1/tx_johadvbesearch.php']);
}
?>
